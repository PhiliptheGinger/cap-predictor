...
def k_fold_cross_validation(model_class, df, selected_models, learning_rate, epochs, k=5, weight_decay=1e-4, num_layers=1, 
                            dropout_rate=0.0, bidirectional=False, optimizer=None, criterion=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        train_liquid_nn(model, optimizer, criterion, X_train, y_train, epochs=int(epochs))
        ...
        with torch.no_grad():
            ...
            logger.debug(f"Shape of val_predictions: {val_predictions.shape}")
            logger.debug(f"Shape of y_val: {y_val.shape}")

            assert val_predictions.shape == y_val.shape, f"Shape mismatch: val_predictions {val_predictions.shape}, y_val {y_val.shape}"
