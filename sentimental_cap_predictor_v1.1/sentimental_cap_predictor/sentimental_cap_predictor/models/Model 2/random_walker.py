...
from sentimental_cap_predictor.config import PROCESSED_DATA_DIR, INTERIM_DATA_DIR, RAW_DATA_DIR, MODELING_DIR
...
try:
    from stochastic_grid_search import simulate_CIR, simulate_GBM, simulate_JumpDiffusion
...
def generate_random_walks(model, steps, num_simulations=100, model_type='sarima', **kwargs):
    ...
    for _ in tqdm(range(num_simulations), desc=f"Generating random walks for {model_type}"):
        if model_type == 'sarima':
            ...
            sim = simulate_CIR(*kwargs.values(), T=1, N=steps)
        elif model_type == 'gbm':
            sim = simulate_GBM(*kwargs.values(), T=1, N=steps)
        elif model_type == 'jump':
            sim = simulate_JumpDiffusion(*kwargs.values(), T=1, N=steps)
        else:
...
def main(
    ...
    steps = len(test_data)
    num_simulations = 100
    sarima_simulations = generate_random_walks(sarima_model, steps, num_simulations, model_type='sarima')

    logger.info("Applying L2 regularization to SARIMA simulations.")
    sarima_optimal_prediction = apply_l2_regularization(sarima_simulations, true_values=test_data)
    ...
    cir_simulations = generate_random_walks(None, steps, num_simulations, model_type='cir', **cir_params)

    logger.info("Performing GBM simulations.")
    gbm_params = {'mu': 0.1, 'sigma': 0.2, 'S0': 1.0}
    gbm_simulations = generate_random_walks(None, steps, num_simulations, model_type='gbm', **gbm_params)

    logger.info("Performing Jump Diffusion simulations.")
    jump_params = {'lambda_': 0.1, 'muJ': 0.1, 'sigmaJ': 0.2, 'S0': 1.0}
    jump_simulations = generate_random_walks(None, steps, num_simulations, model_type='jump', **jump_params)
    ...
    cir_optimal_prediction = apply_l2_regularization(cir_simulations, true_values=test_data)

    logger.info("Applying L2 regularization to GBM simulations.")
    gbm_optimal_prediction = apply_l2_regularization(gbm_simulations, true_values=test_data)

    logger.info("Applying L2 regularization to Jump Diffusion simulations.")
    jump_optimal_prediction = apply_l2_regularization(jump_simulations, true_values=test_data)

    # Save Predictions and Results
    df_test = pd.DataFrame({
        'TrueValues': test_data,
        'SARIMA': sarima_predictions,
        'Optimal_SARIMA': sarima_optimal_prediction,
        'Optimal_CIR': cir_optimal_prediction,
        'Optimal_GBM': gbm_optimal_prediction,
        'Optimal_Jump': jump_optimal_prediction,
    })
